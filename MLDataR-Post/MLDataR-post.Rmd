...
For Machine Learning models, it is generally recommended to split your data into a training and testing set, or if you are using hyperparameter tuning and updating your model, a training / test and validation set. Other methods are available, such a K Fold Cross Validation, however we will stick to a basic training and testing split for the purposes of this demo. To do this, and to make sure that the results are repeatable, we will use the `set.seed(123)` value - which essentially says when we are randomly splitting this data, make sure that the random pattern is the same as the tutorial i.e. give me the same split as this blog:
...
training_message <- function(){
  message(cat('The training set has: ', 
              nrow(training), ' examples and the testing set has:',
              nrow(testing),'.\nThis split has ',
              paste0(format(100*split_prop),'%'), 
              ' in the training set and ',paste0(format(100*testing_prop), '%'), 
              ' in the testing set.',sep=''))
}
training_message()
```
...
Let's convert the probabilities from the fitted GLM model into odds ratios (ORs). An OR measures the association between an exposure and an outcome.^[Szumilas M. (2010). Explaining odds ratios. Journal of the Canadian Academy of Child and Adolescent Psychiatry = Journal de l'Academie canadienne de psychiatrie de l'enfant et de l'adolescent, 19(3), 227–229.] In this case, the OR represents the odds of heart disease will occur given a particular condition, compared to the odds of heart disease occurring in the absence of that condition. In order words, the ORs will tell us, "for every change in 1 unit, what is the increase in odds of getting heart disease?"
...
```{r, message = FALSE}
tidy_oddsplot <- OddsPlotty::odds_plot(lr_hd_fit$fit,
...
tidy_oddsplot <- tidy_oddsplot$odds_plot +
  ...
  geom_text(label=round(tidy_oddsplot$odds_plot$data$OR,digits=3),
            hjust=-0.5, vjust=1, cex=2.8)
...
cm <- ConfusionTableR::binary_class_cm(predictions$.pred_class, 
...
As I run many ML experiments I wanted a way to store this data into a record level extract. I actually stored this in a model metrics table on Postgres SQL database, but could be stored in any propriatory system. To get these metrics easily in record level, the package I created helps you do that with ease:
